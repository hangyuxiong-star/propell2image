---
name: propell2image-client
description: 通过 propell2image 的 AI Agent 进行多模态内容创作。覆盖场景包括：AI 图片生成、AI 视频生成、AI 音频生成、3D 模型生成、多模态内容编辑、画布（Canvas）项目管理。当需要调用 propell2image 生成图片/视频/音频/3D 模型、或需要与 propell2image 画布对话时应触发此技能。
---

# propell2image Client

propell2image 是一个多模态 AI 内容创作系统（FastAPI + LangGraph ReAct Agent + React + Excalidraw）。通过 `curl` 向 propell2image 发送指令、消费 SSE 流式响应、追踪工具调用结果、获取生成的媒体文件。

## 环境配置

```bash
export PROPELL2IMAGE_HOST="localhost"   # 必需，propell2image 主机地址
export PROPELL2IMAGE_PORT="8000"        # 可选，默认 8000
```
...
```bash
curl -N --no-buffer \
  -X POST "http://${PROPELL2IMAGE_HOST:-localhost}:${PROPELL2IMAGE_PORT:-8000}/api/chat" \
...
propell2image 后端有完整的 LangGraph ReAct Agent 负责实际创作，你负责的是**指令传达和结果获取**。

**要做的三件事：**
1. **传话** — 把用户的原始需求原封不动发给 propell2image Agent
...
| 500 | 服务器错误 | 检查 propell2image 后端是否正常运行 |
| 连接拒绝 | 服务未启动 | 确认 `PROPELL2IMAGE_HOST:PORT` 可访问 |
